...
void print(const Container & c){
    ...
    // <<>> (1 line)
    std::copy(c.begin(), c.end(), std::ostream_iterator<typename Container::value_type>(std::cout, ", "));//kopiuje do ostream iterator
    std::cout << std::endl;//dodaje przecinek i spacje
}
...
void toLowerAlpha(std::string & s) {
    ...
    s.erase(std::remove_if(s.begin(), s.end(), [](unsigned char c) {
        //przesuwa wszystkie znaki ktore nie są literami na koniec stringa i zwraca
...
// <<>> (several lines)
    std::string original1 = word1;
    std::string original2 = word2;
