...
int main(){
  ...
  std::sort(v.begin(), v.end(), [center](int a, int b) {
      int distanceA = std::abs(center - a);
      int distanceB = std::abs(center - b);
  ...
  auto positiveEvenFirst= [](int a, int b) {
      ...
      bool parzystoscA;
      bool parzystoscB;
      if (a > 0) znakA = 1;//dodatnie
      else if (a == 0) znakA = 2;//zero
      else znakA = 3; // ujemne
      if (b > 0) znakB = 1;//dodatnie
      else if (b == 0) znakB = 2;//ujemne
      else znakB = 3; // zero

      if (znakA != znakB) return znakA < znakB;
      ...
      if (parzystoscA != parzystoscB) return parzystoscA;
      return a < b;
  };                  // 2
  ...
  };  /// 3 bez parametrow bo pobiera z programu te zmienne
  ...
  int start = 5, step =2;
    auto arithmeticGenerator = [base = start, &step, count = 0]() mutable {
        int current = base + (count * step);
        count++;
        return current;
    };// [ 4 ]
  ...
  auto l1_norm = [](const auto& container) {//stl nam to umozliwia
      return std::accumulate(container.begin(), container.end(), 0.0, [] (double suma, double element) {
        suma += std::abs(element);
          return suma;
      });
  }; ;   //tworzymy kontener auto (szablon)                      // [ 5 ]
